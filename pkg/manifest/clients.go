...
var allClients = []ClientManifest{
	...
	},
	{
		...
		Candidates: []ConfigCandidate{
			{
				// Canonical current path; may be a symlink to a resolved target.
				Label:        "mcp-config",
			...
			{
				// Post-migration fallback observed in issue #60 after Antigravity CLI install.
				...
				Precedence:      15,
				Deprecated:      true,
				ReplacedBy:      "mcp-config",
				DeprecationNote: "post-migration fallback path observed after Antigravity CLI installation (issue #60)",
			},
			{
				// Pre-I/O 2026 path; deprecated since Antigravity renamed its config location.
				Label:           "legacy-pre-io26",
				...
				Deprecated:      true,
				ReplacedBy:      "mcp-config",
				DeprecationNote: "pre-I/O 2026 Antigravity CLI MCP config path",
			},
			{
				Label:        "workspace",
				PathTemplate: "{{.Workspace}}/.agents/mcp_config.json",
				Platforms:    []string{"darwin", "linux"},
				Scope:        ScopeWorkspace,
				Format:       FormatJSON,
				MutationKind: MutationMCPServers,
